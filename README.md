...
필요하고 약 88초 걸립니다 — [3절](#3-선택-법률-코퍼스-색인조회-data-platform)).
...
**약 88초면 질의까지 됩니다**(직접 빌드하면 약 32분). 색인을 새로 만드는 대신 발행된 것을
받아 설치하기 때문입니다. 다섯 단계 전부 멱등이라 다시 돌려도 안전합니다(재실행 7초).
...
- **clone 후 한 번 `make quickstart`.** 훅·스킬을 세우고 코퍼스를 바로 쓸 수 있게 합니다
  (약 88초). 훅·스킬만 필요하면 `make hooks && make skills`입니다. 둘 다 git이 clone에 담아 보내지 못하는
